<?php
    ...
    if($intentos<3){
        if (password_verify($pass, $pass_correcta)) {
            ...
            $query3 = mysqli_prepare($conn,"UPDATE usuario SET Intentos=? WHERE `Used ID`=?") or die (mysqli_error($conn));
            ...
            $query4 = mysqli_prepare($conn,"UPDATE usuario SET Intentos=? WHERE `Used ID`=?") or die (mysqli_error($conn));
            mysqli_stmt_bind_param($query4, "is", $intentos4, $us);
            $us = $user;
            $intentos4=$intentos+1;
            mysqli_stmt_execute($query4) or die (mysqli_error($conn));
            mysqli_close($conn);
            $conn = mysqli_connect($hostname,$username,$password,$db);
            if ($conn->connect_error) {
                die("Database connection failed: " . $conn->connect_error);
